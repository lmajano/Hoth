...
<!------------------------------------------- HELPERS ------------------------------------------->
	...
	<cffunction name="getMockDatasource" access="private" output="false" returnType="mockbox.system.core.db.DatasourceBean" hint="I will return to you a datasourceBean according to the mocking parameters sent">
		...
		<cfscript>
			...
			return getMockBox().createMock("mockbox.system.core.db.DatasourceBean").init(arguments);
		</cfscript>
	</cffunction>
	
	<!--- getMockConfigBean --->
	<cffunction name="getMockConfigBean" access="private" output="false" returnType="mockbox.system.core.collections.ConfigBean" hint="I will return to you a configBean according to the mocking configuration structure you send in">
		<cfargument name="configStruct" type="struct" required="false" default="#structnew()#" hint="A memento of name-value pairs to init">
	    <cfscript>
			return getMockBox().createMock("mockbox.system.core.collections.ConfigBean").init(arguments.configStruct);
		</cfscript>
	</cffunction>
    ...
    <cffunction name="getMockPlugin" output="false" access="private" returntype="any" hint="Get a plugin mocked with capabilities">
    	...
    	<cfscript>
    		...
    		// Create Mock Objects
			var mockController 		= mockBox.createEmptyMock("mockbox.system.testing.mock.web.MockController");
			var mockRequestService 	= mockBox.createEmptyMock("mockbox.system.web.services.RequestService");
			var mockLogBox	 		= mockBox.createEmptyMock("mockbox.system.logging.LogBox");
			var mockLogger	 		= mockBox.createEmptyMock("mockbox.system.logging.Logger");
			var mockFlash		 	= mockBox.createMock("mockbox.system.web.flash.MockFlash").init(mockController);
			var mockCacheBox   		= mockBox.createEmptyMock("mockbox.system.cache.CacheFactory");
	...
	<!--- getColdboxOCM --->
    <cffunction name="getColdboxOCM" access="private" output="false" returntype="any" hint="Get ColdboxOCM: mockbox.system.cache.CacheManager or new CacheBox providers mockbox.system.cache.IColdboxApplicationCache" colddoc:generic="mockbox.system.cache.IColdboxApplicationCache">
	...
	<!--- getUtil --->
	<cffunction name="getUtil" access="private" output="false" returntype="mockbox.system.core.util.Util" hint="Create and return a util object">
		<cfreturn CreateObject("component","mockbox.system.core.util.Util")/>
	</cffunction>
